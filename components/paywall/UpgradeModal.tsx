...
export default function UpgradeModal() {
  ...
  return (
    <AnimatePresence>
      <motion.div
        className="fixed inset-0 z-[90] bg-ink/40 grid place-items-center px-4 py-6"
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        exit={{ opacity: 0 }}
      ...
      >
        {/* No backdrop-blur, no transform-based entrance (opacity only) —
            both were real, documented contributors to broken touch/scroll
            behavior on some mobile browsers combined with nested overflow.
            The close button now lives in its own fixed header rather than
            buried inside the scrollable content — previously, if that
            inner scroll area failed to scroll on a given device, there was
            no way to close the modal at all. */}
        <motion.div
        ...
        >
          <div className="flex items-center justify-between px-5 py-3 border-b border-line shrink-0">
            <span className="font-display text-sm font-semibold flex items-center gap-2">
              <Sparkles size={16} className="text-teal-text" /> Unlock the full story
            </span>
            <button onClick={closeUpgradeModal} className="text-ink-soft hover:text-ink" aria-label="Close">
              <X size={20} />
            </button>
          </div>

          <div className="overflow-y-auto grid md:grid-cols-[1fr_1.4fr]">
            <div className="bg-teal text-white p-6 sm:p-8 flex flex-col justify-between">
              <div>
